...
FEM_FILE = "<name_fem_file.fem>"  # Path to the FEM file modified as needed
GRID_POINT = np.array([])  # Array of node IDs to be extracted modified as needed
...
class FEMParser:
    ...
    def _parse_file(self):
        ...
        # Parse coordinates
        if (
            x_coords_start is not None
            and y_coords_start is not None
            and z_coords_start is not None
        ):
            x_coords = self._parse_coordinate_section(
                lines, x_coords_start, y_coords_start
            )
            y_coords = self._parse_coordinate_section(
                lines, y_coords_start, z_coords_start
            )
    ...
    def _parse_coordinate_section(
        self, lines: List[str], start_line: int, end_marker: int
    ) -> List[float]:
...
def main():
    ...
    for node_id in interface_nodes:
        ...
        if coords:
            print(
                f"{node_id:4d}: {coords[0]:10.8f}, {coords[1]:10.8f}, {coords[2]:10.8f}"
            )
