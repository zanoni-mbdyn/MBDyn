...
% Needed for tecman-hydrodynamic_plain_bearing2.tex
...
that are distributed verbatim in Appendix~\ref{appendix:integrators1},~\ref{appendix:integrators2},
respectively.
...
The \texttt{SubFrom} method takes the same arguments of \texttt{AddTo},
but subtracts instead of adding.
...
\end{align}
The correction term $\T{\beta} \times \T{v}$ is already included
in the equations of motion by the \texttt{AutomaticStructuralElem}
element associated with the underlying node;
...
present information associated with other structural nodes 
...
\chapter{Constraints (aka Joints)}
...
\subsection{Beam Slider Joint}
\label{sec:BeamSliderJoint}
The beam slider joint allows a point attache to a node, 
$\T{x}_c = \T{x}_b+\TT{R}_b\T{f}_b$ to slide along a sequence of three-node beam
elements.
The current contact position on the beam is 
$\T{x}=\sum_{i=1}^3N_i(s)(\T{x}_i + \TT{R}_i\T{f}_i)$.

\paragraph{Files.} \
It is implemented in files

\begin{tabular}{l}
\texttt{mbdyn/struct/beamslider.h} \\
\texttt{mbdyn/struct/beamslider.cc}
\end{tabular}

\paragraph{Unknowns.} \
\begin{itemize}
\item $s$ position along the beam
\item $\T{F}$ reaction force vector (three components)
\item $\T{M}$ reaction moment vector (no components for the \emph{spherical} slider, two components for the
\emph{classical} slider, three components for the \emph{spline} slider, currently not implemented)
\end{itemize}
If friction is present additional unknowns depend on the chosen friction model.


\paragraph{Definitions.}
\begin{itemize}
\item $\T{x}_{ti} = \T{x}_i + \TT{R}_i\T{f}_i$ 
\item $\T{l} = \sum_{i=1}^3 N_{i/s}(s) \T{x}_{ti}$ beam tangent
\item $\T{e}_{3a} = \T{l} / \sqrt{\T{l}\cdot\T{l}}$ unit beam tangent
\item $\T{x}_c = \T{x}_b+\TT{R}_b\T{f}_b$ body sliding point position
\item $\T{x}=\sum_{i=1}^3N_i(s)(\T{x}_i + \TT{R}_i\T{f}_i)$ contact point position on the beam
\item $\T{v}_c = \dot{\T{x}}_b + \T{\omega}_b\times\TT{R}_b\T{f}_b$ body sliding point velocity
\item $\T{v}=\sum_{i=1}^3N_i(s)(\dot{\T{x}}_i + \T{\omega}_i\times\TT{R}_i\T{f}_i)$ contact point velocity on the beam
\end{itemize}

\paragraph{Equations.}
\begin{itemize}
\item 1: $\T{F} \cdot \T{l} / \texttt{dCoef}= 0$
\item 2--4: $(\T{x}_c - \T{x}) / \texttt{dCoef} = \T {0}$
\end{itemize}

If \emph{classical} two additiona equation:
\begin{itemize}
\item 5: $\TT{R}_b(2) \cdot \T{l} = 0$
\item 6: $\TT{R}_b(3) \cdot \T{l} = 0$
\end{itemize}

If \emph{spline}: not implemented

\paragraph{Friction}
If friction is present the friction coefficient $f$ is computed as a function of
the relative sliding velocity $v_{rel} = \T{e}_{3a} \cdot \T{v}_{rel}$ where $\T{v}_{rel} = \T{v}_c - \T{v}$.
The tangent friction force component is $F_3 = f * S_{hc} * \sqrt(\T{F}\cdot\T{F})$
and his direction is that of $\T{e}_{3a}$, so that the resultant reaction force is 
$\T{F}_{res} = \T{F} - \T{e}_{3a} F_3$

\paragraph{Friction linearization.}
\begin{align*}
\Delta v_{rel} = \T{e}_{3a} \cdot \Delta\T{v}_{rel} + \T{v}_{rel}\cdot\Delta\T{e}_{3a}
\end{align*}
\begin{align*}
\T{e}_{3a} \cdot \Delta \T{v}_{rel} = & \T{e}_{3a} \cdot \Delta \T{x}_c\\
& -\sum_{i=1}^3 \T{e}_{3a} \cdot (\dot{\T{x}}_i + \T{\omega}_i\times\TT{R}_i\T{f}_i) N_{i/s}\Delta s\\
& -\sum_{i=1}^3 N_i \T{e}_{3a} \cdot \Delta \dot{\T{x}}_i \\
& -\sum_{i=1}^3 N_i (\TT{R}_i \T{f}_i)\times \T{e}_{3a} \Delta \T{\omega}_i\\
& -\sum_{i=1}^3 N_i (\TT{R}_i\T{f}_i)\times \T{\omega}_i \times \T{e}_{3a} \texttt{dCoef} \Delta \dot{\T{g}}
\end{align*}
where, as explained in Sec.~\ref{sec:NodalRotation}, $\Delta \T \omega = \Delta \dot{\T g}-
\T \omega_{\mathrm{ref}}\times\Delta \dot{\T g} * \texttt{dCoef}$. Furthermore,
\begin{align*}
\T{v}\Delta\T{e}_{3a} =
\left(\frac{\T{v}}{\sqrt{\T{l}\cdot\T{l}}}-\frac{v_{rel}\T{l}}{(\T{l}\cdot\T{l})^{3/2}}\right)
\cdot\Delta\T{l}
\end{align*}
with
\begin{align*}
\Delta\T{l} = &\sum_{i=1}^3 N_{i/ss}\T{x}_{ti}\Delta\dot{s}\\
&+\sum_{i=1}^3N_{i/s}\TT{I}\texttt{dCoef}\Delta\dot{\T{x}}_i\\
&+\sum_{i=1}^3N_{i/s}(-\TT{R}_i\T{f}_i)\times\texttt{dCoef}\Delta\dot{\T{g}}_i
\end{align*}
...
\begin{align}
	\sqbr{\cvvect{
		\plbr{c\T{K}_{\T{l}} + \T{K}_{\dot{\T{l}}}}
		\\
		\T{f}_1 \times \plbr{c\T{K}_{\T{l}} + \T{K}_{\dot{\T{l}}}}
		\\
		-\plbr{c\T{K}_{\T{l}} + \T{K}_{\dot{\T{l}}}}
		\\
		-\T{f}_2 \times \plbr{c\T{K}_{\T{l}} + \T{K}_{\dot{\T{l}}}}
	}}
	\delta\dot{\T{x}}_1
	& \nonumber \\ \mbox{}
	+
	\sqbr{\cvvect{
		- \plbr{\plbr{c\T{K}_{\T{l}} + \T{K}_{\dot{\T{l}}}} \T{f}_1\times{} - c \T{K}_{\dot{\T{l}}} \plbr{\T{f}_1 \times \T{\omega}_1}\times{}}
		\\
		- \T{f}_1 \times \plbr{\plbr{c\T{K}_{\T{l}} + \T{K}_{\dot{\T{l}}}} \T{f}_1\times{} - c \T{K}_{\dot{\T{l}}} \plbr{\T{f}_1 \times \T{\omega}_1}\times{}}
			- c \T{F} \times \T{f}_1 \times 
		\\
		\plbr{\plbr{c\T{K}_{\T{l}} + \T{K}_{\dot{\T{l}}}} \T{f}_1\times{} - c \T{K}_{\dot{\T{l}}} \plbr{\T{f}_1 \times \T{\omega}_1}\times{}}
		\\
		\T{f}_2 \times \plbr{\plbr{c\T{K}_{\T{l}} + \T{K}_{\dot{\T{l}}}} \T{f}_1\times{} - c \T{K}_{\dot{\T{l}}} \plbr{\T{f}_1 \times \T{\omega}_1}\times{}}
	}}
	\delta\dot{\T{g}}_1
	& \nonumber \\ \mbox{}
	+
	...
	+
	\sqbr{\cvvect{
		\plbr{\plbr{c\T{K}_{\T{l}} + \T{K}_{\dot{\T{l}}}} \T{f}_2\times{} - c \T{K}_{\dot{\T{l}}} \plbr{\T{f}_2 \times \T{\omega}_2}\times{}}
		\\
		\T{f}_1 \times \plbr{\plbr{c\T{K}_{\T{l}} + \T{K}_{\dot{\T{l}}}} \T{f}_2\times{} - c \T{K}_{\dot{\T{l}}} \plbr{\T{f}_2 \times \T{\omega}_2}\times{}}
		\\
		- \plbr{\plbr{c\T{K}_{\T{l}} + \T{K}_{\dot{\T{l}}}} \T{f}_2\times{} - c \T{K}_{\dot{\T{l}}} \plbr{\T{f}_2 \times \T{\omega}_2}\times{}}
		\\
		- \T{f}_2 \times \plbr{\plbr{c\T{K}_{\T{l}} + \T{K}_{\dot{\T{l}}}} \T{f}_2\times{} - c \T{K}_{\dot{\T{l}}} \plbr{\T{f}_2 \times \T{\omega}_2}\times{}}
			+ c \T{F} \times \T{f}_2 \times{}
	}}
	\delta\dot{\T{g}}_2
	&=
	\cubr{\cvvect{
...
\end{align}
\end{subequations}
since the relative velocity associated with the rotation of $\T{f}_1$,
...
\chapter{Hydrodynamic Plain Bearing Element}
\input{tecman-hydrodynamic_plain_bearing2}

\chapter{Module MFtire}
\input{module-MFtire}
...
Tel.: ++39 02 2399 8309 \\
Fax: ++39 02 2399 8334 \\
E-mail: \htmladdnormallink{\texttt{pierangelo.masarati@polimi.it}}{mailto:pierangelo.masarati@polimi.it} \\
Web: \htmladdnormallink{\texttt{https://www.mbdyn.org/}}{https://www.mbdyn.org/} \\
Web: \htmladdnormallink{\texttt{https://www.aero.polimi.it/en/staff/pierangelo-masarati}}{https://www.aero.polimi.it/en/staff/pierangelo-masarati} \\
