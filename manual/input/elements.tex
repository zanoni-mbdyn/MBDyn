...
in the real joint, one should set it to a small number $\neq 0$ \textcolor{red}{[FIXME: should preload be $ > 0$ ? In that case, should we check for negative (or null) values? Should it be a drive caller?]} in order to deal with ill-conditioning of the friction computation occurring whenever the real joint reaction force is almost null;
