...
\emph{WARNING: This procedure has been tested with MSC NASTRAN 2024.1}
...
eigenanalysis properties data (SOL103) and/or non-linear static loads (SOL106). 

\item Add the following \kw{OUTPUT} in the case control section, to request
the output of displacements and eigenvectors for all nodes in \texttt{.op2} format: 
	\begin{verbatim}
		DISPLACEMENT(PLOT) = ALL 
		VECTOR(PLOT) = ALL
	\end{verbatim}
\item Add the following \kw{PARAM} in the bulk data section, to request 
	the \texttt{.op2} file format and the output of the initial geometry:
	\begin{verbatim}
		PARAM,POST, -1
		PARAM,OGEOM,YES
	\end{verbatim}
\item Complete the NASTRAN input file by including some specific 
	\kw{ALTER} cards at the beginning of the input file.
	In detail:
	\begin{enumerate}
	\item \texttt{MBDyn\_NASTRAN\_alter\_1.nas} contains
		...
		of the input file \textit{WARNING: Deprecated};

		\emph{FIXME: I don't know how to use static shapes only}.

	\item \texttt{MBDyn\_NASTRAN\_alter\_2.nas} contains
	...
	\item \texttt{MBDyn\_NASTRAN\_alter\_SOL103\_2024.nas} contains
		\kw{ALTER} definitions for eigenanalysis solutions (SOL103);
		an appropriate eigenanalysis method, with the related
		data card must be provided in the case control 
		and in the bulk data sections of the input file;

		\emph{Note: this works; see} \texttt{tests/joints/modal/beam.README} and 
		\texttt{tests/joints/modal/goland-wing}.

	\item \texttt{MBDyn\_NASTRAN\_alter\_SOL106\_2024.nas} contains
		\kw{ALTER} definitions for a non-linear static analysis followed by an 
		eigenanalysis solutions (SOL106);

		\emph{Note: this works; see} \texttt{tests/joints/modal/shell-blade}
	...
	Both \kw{SOL 103} and \kw{SOL 106} produce one binary file \nt{mbdyn\_modal.mat} which
	contains the modal mass and stiffness matrices and the lumped nodal masses.  
	The deformed configuration and the mode shapes are written in the \texttt{.op2} file. 
\item Run NASTRAN.

\item Run the tool \texttt{femgen}, which transforms the above binary files
	into the \nt{fem\_data\_file} using the syntax: 
	\begin{verbatim}
		femgen <{out_nastran}> [-o <{fem_data_file}>]   	
	\end{verbatim}
	where <\nt{out\_nastran}> is the base name of the output files (without extension) 
	and <\nt{fem\_data\_file}> is the name of the \texttt{.fem} output file to be generated.
	If the \texttt{-o} option is omitted, the output file name defaults to
	<\nt{out\_nastran}.\texttt{fem}>. 

\item To obtain the coordinates of the desired interface nodes, you can use the python script
	\texttt{parse\_fem.py} provided in directory \texttt{etc/modal.d/} modified as needed with the
	name of the \texttt{.fem} file and the array of node ids to be extracted. 
