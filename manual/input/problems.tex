...
If the option \kw{direction} is set to \kw{newton}, an inexact Newton method is applied. This method uses iterative linear solvers to compute the Newton direction. The following linear solvers provided by \href{https://trilinos.github.io/belos.html}{Belos} are supported:
\begin{description}
\item[\kw{Block GMRES}] Block restarted generalized minimal residual
\item[\kw{Pseudo Block GMRES}] 
\item[\kw{Pseudo Block Stochastic CG}]
\item[\kw{GCRODR}] 
\item[\kw{RCG}] 
\item[\kw{MINRES}] 
\item[\kw{TFQMR}] 
\item[\kw{BiCGStab}] 
\item[\kw{Fixed Point}] 
\item[\kw{TPETRA GMRES}] 
\item[\kw{TPETRA GMRES PIPELINE}] 
\item[\kw{TPETRA GMRES SINGLE REDUCE}] 
\item[\kw{TPETRA GMRES S-STEP}] 
\end{description}
...
\begin{Verbatim}[commandchars=\\\{\}]
    \bnt{card} ::= \kw{linear solver} :
            ...
            | \kw{belos} | \kw{amesos} | \kw{siconos dense} | \kw{siconos sparse} \}
...
\paragraph{Belos}
In contrast to most other solvers, \kw{belos} is an iterative linear solver based on
