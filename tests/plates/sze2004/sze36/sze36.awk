# $Header: /var/cvs/mbdyn/mbdyn/mbdyn-1.0/tests/plates/sze2004/sze36/sze36.awk,v 1.8 2017/01/12 15:08:46 masarati Exp $
#
...
function header(out) {
	...
	print "# " >> out;
	print "# 3.6 Pinched cylindrical shell mounted over rigid diaphragms" >> out;
	print "#" >> out;
	print "# GENERATED BY 'sze36.awk' - DO NOT EDIT" >> out;
	print "" >> out;
}
...
BEGIN {
	...
	print "# symmetry constraints along {x,z}={R,0} & {0,R}" >> out;
	for (ny = 1; ny < N_Y; ny++) {
		y = (L/2)*(ny/N_Y);
	...
	print "# symmetry constraint in {x,y,z}={R,0,0}" >> out;
	printf "joint: %d, total pin joint,\n", 0 >> out;
	printf "\t%d, position, reference, node, null,\n", 0 >> out;
	print "\t\tposition orientation, reference, 0, eye," >> out;
	print "\t\trotation orientation, reference, 0, eye," >> out;
	printf "\tposition, reference, 0, %+.8e, %+.8e, %+.8e,\n", R, 0., 0. >> out;
	print "\tposition constraint, 1, 0, 1, null," >> out;
	print "\torientation constraint, 1, 1, 0, null;" >> out;
	print "" >> out;

	print "# symmetry constraint in {x,y,z}={0,0,R}" >> out;
	printf "joint: %d, total pin joint,\n", N_C >> out;
	printf "\t%d, position, reference, node, null,\n", N_C >> out;
	print "\t\tposition orientation, reference, 0, eye," >> out;
	print "\t\trotation orientation, reference, 0, eye," >> out;
	...
	print "# symmetry constraint in {x,y,z}={R,L/2,0}" >> out;
	printf "joint: %d, total pin joint,\n", DELTA_N_Y*N_Y >> out;
	printf "\t%d, position, reference, node, null,\n", POINT_B >> out;
	print "\t\tposition orientation, reference, 0, eye," >> out;
	print "\t\trotation orientation, reference, 0, eye," >> out;
	...
	print "# symmetry constraint in {x,y,z}={0,L/2,R}" >> out;
	printf "joint: %d, total pin joint,\n", POINT_A >> out;
	printf "\t%d, position, reference, node, null,\n", POINT_A >> out;
	print "\t\tposition orientation, reference, 0, eye," >> out;
	print "\t\trotation orientation, reference, 0, eye," >> out;
	...
	print "\t# position constraint, 1, 1, 1, 0., 0., -1., cosine, 0., pi/1., 85./2, half, 0.," >> out;
	print "\torientation constraint, 1, 1, 1, null;" >> out;
	print "" >> out;

	print "# symmetry/diaphragm constraints along y=0 & y=L" >> out;
	for (nc = 1; nc < N_C; nc++) {
		theta = 3.14159265358979323846/2*nc/N_C;
		...
		printf "\t\t1, %+.8e, %+.8e, %+.8e,\n", x, 0., z >> out;
		print "\tposition constraint, 1, 0, 1, null," >> out;
		print "\torientation constraint, 0, 0, 0, null;" >> out;
		print "" >> out;
