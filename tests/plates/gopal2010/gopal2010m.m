...
fd = header('gopal2010m_m.nod');

OFFSET = 100;
for il = 0:NL,
	for ib = 0:NB,
		if (il == 0 || il == NL || ib == 0 || ib == NB),
			...
			fprintf(fd, 'structural: %d, dynamic displacement,\n', OFFSET*il + ib);
			fprintf(fd, '        reference, PITCH, %e*B, %e*L, %e,\n', ib/NB - 0.25, il/NL, 0.);
			fprintf(fd, '        reference, PITCH, null;\n');
		end
		fprintf(fd, '\n');
	end
...
for il = 0:NL,
	for ib = 0:NB,
		...
		fprintf(fd, '        %e*B*L*H*RHO', m);
		if (il == 0 || il == NL || ib == 0 || ib == NB),
			fprintf(fd, ',\n')
			fprintf(fd, '        reference, node, %e*B, %e*L, 0.,\n', xb, xl);
			fprintf(fd, '        diag, %e*B*L^3*H*RHO, %e*B^3*L*H*RHO, (%e*B^3*L + %e*B*L^3)*H*RHO', Jz, Jx, Jz, Jx);
		end
		fprintf(fd, ';\n')
		fprintf(fd, '\n');
	end
...
for il = 1:NL,
	for ib = 1:NB,
		...
		fprintf(fd, 'membrane4eas: %d, %d, %d, %d, %d,\n', ...
			label, node1, node2, node3, node4);
		fprintf(fd, '        isotropic, E, E, nu, NU, thickness, H,\n');
		fprintf(fd, '        prestress, PS, PS, 0.;\n');
	end
end

footer(fd);

fd = header('gopal2010m_ext_m.elm');

mb_file = 'gopal2010m_shell_mb.dat';
cfd_file = 'gopal2010_shell_surface_grid.dat';
mapping_file = 'gopal2010m_shell_H.dat';
...
for il = 0:NL,
	for ib = 0:NB,
		if (il == 0 || il == NL || ib == 0 || ib == NB),
			...
			fprintf(fd, '                %4d, membrane, %d, %d, %d, %d, .25*H, -.25*H,\n', ...
				OFFSET*il + ib,
				OFFSET*(il - 1) + ib,
				OFFSET*(il + 1) + ib,
				OFFSET*il + ib - 1,
