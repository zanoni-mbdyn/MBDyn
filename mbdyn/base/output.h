...
namespace MBDynNetCDF {
     ...
     template <sp_grad::index_type iNumRows, sp_grad::index_type iNumCols>
     struct is_NcVar<sp_grad::SpMatrix<doublereal, iNumRows, iNumCols>> {
          static constexpr bool value = true;
     };
}
/* OutputHandler - begin */
...
public:
        ...
        template <sp_grad::index_type iNumRows, sp_grad::index_type iNumCols>
        void WriteNcVar(const MBDynNcVar& ncVar, const sp_grad::SpMatrixBase<doublereal, iNumRows, iNumCols>& mat);

        template <typename T, typename std::enable_if<std::is_base_of<sp_grad::SpMatrixBase<doublereal, T::iNumRowsStatic, T::iNumColsStatic>, T>::value, bool>::type = true>
        void WriteNcVar(const MBDynNcVar& ncVar, const T& matA) {
             const sp_grad::SpMatrixBase<doublereal, T::iNumRowsStatic, T::iNumColsStatic>& mat = matA;
             WriteNcVar(ncVar, mat);
        }

        template <class Tvar, typename std::enable_if<std::is_arithmetic<Tvar>::value, bool>::type = true>
        void
        WriteNcVar(const MBDynNcVar&, const Tvar&);

        template <class Tvar, class Tstart, typename std::enable_if<std::is_arithmetic<Tvar>::value, bool>::type = true>
        void
        ...
        const Dimensions phys_dim, const std::string& description)
{
        ...
        if constexpr (std::is_same<T, integer>::value) {
        ...
        } else if constexpr (std::is_same<T, Mat3x3>::value) {
             ...
             static_assert(T::iNumRowsStatic > 0);
             static_assert(T::iNumColsStatic > 0);

             attrs[1] = AttrVal("type", "SpMatrix");
             dims.resize(3);

             const std::string dimnrows = "matrix rows "s + std::to_string(T::iNumRowsStatic);
             const std::string dimncols = "matrix columns "s + std::to_string(T::iNumColsStatic);

             dims[1] = GetDim(dimnrows);

             if (dims[1].isNull()) {
                  dims[1] = CreateDim(dimnrows, T::iNumRowsStatic);
             }

             dims[2] = GetDim(dimncols);

             if (dims[2].isNull()) {
                  dims[2] = CreateDim(dimncols, T::iNumColsStatic);
             }

             return CreateVar(name, MbNcDouble, attrs, dims);
        }
}
...
template <sp_grad::index_type iNumRows, sp_grad::index_type iNumCols>
void OutputHandler::WriteNcVar(const MBDynNcVar& Var_Var, const sp_grad::SpMatrixBase<doublereal, iNumRows, iNumCols>& mat)
{
     std::vector<size_t> ncStart(3, 0);
     ncStart[0] = GetCurrentStep();
     std::vector<size_t> ncCount = {1, iNumRows, iNumCols};
     Var_Var.putVar(ncStart, ncCount, mat.begin());
}
#endif /* USE_NETCDF */
