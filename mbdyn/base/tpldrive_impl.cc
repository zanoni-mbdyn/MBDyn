...
public:
        ...
        Read(const DataManager* pDM, MBDynParser& HP) override {
                ...
                SAFENEWWITHCONSTRUCTOR(pTplDC, ZeroTplDriveCaller<T>, ZeroTplDriveCaller<T>(pDM->pGetDrvHdl()));
...
public:
        ...
        Read(const DataManager* pDM, MBDynParser& HP) override {
                ...
                SAFENEWWITHCONSTRUCTOR(pTplDC,
                        ...
                        SingleTplDriveCaller<T>(pDM->pGetDrvHdl(), pDC, t));
...
public:
        explicit CompTplDriveCaller(const DriveHandler* pDH, std::vector<DriveCaller *>&& dc)
        : TplDriveCaller<T>(pDH), m_dc(std::move(dc))
        {
        ...
        virtual TplDriveCaller<T>* pCopy(void) const override {
                ...
                SAFENEWWITHCONSTRUCTOR(pDC, dc, dc(this->pDrvHdl, std::move(tmpdc)));
...
public:
        explicit CompTplDriveCaller(const DriveHandler* pDH, std::vector<DriveCaller *>&& dc)
             : TplDriveCaller<Mat3x3>(pDH), m_dc(std::move(dc))
        {
                if (m_dc.size() != 9) {
                        throw ErrGeneric(MBDYN_EXCEPT_ARGS);
                }
        ...
        /* copia */
        virtual TplDriveCaller<Mat3x3>* pCopy(void) const override {
                ...
                SAFENEWWITHCONSTRUCTOR(pDC, dc, dc(pDrvHdl, std::move(tmpdc)));
        ...
        inline Mat3x3 Get(const doublereal& dVar) const override {
                Mat3x3 t;
        ...
        inline Mat3x3 GetP(const doublereal& dVar) const override {
                Mat3x3 t;

                for (unsigned i = 0; i < m_dc.size(); i++) {
                        t(i/3 + 1, i%3 + 1) = m_dc[i]->dGetP(dVar);
                }
...
public:
        explicit CompTplDriveCaller(const DriveHandler* pDH, std::vector<DriveCaller *>&& dc)
        : TplDriveCaller<Mat6x6>(pDH), m_dc(std::move(dc))
        {
                if (m_dc.size() != 36) {
                        throw ErrGeneric(MBDYN_EXCEPT_ARGS);
                }
        ...
        /* copia */
        virtual TplDriveCaller<Mat6x6>* pCopy(void) const override {
        ...
        inline Mat6x6 Get(const doublereal& dVar) const override {
                Mat6x6 t;
        ...
        inline Mat6x6 GetP(const doublereal& dVar) const override {
                Mat6x6 t;

                for (unsigned i = 0; i < m_dc.size(); i++) {
                        t(i/6 + 1, i%6 + 1) = m_dc[i]->dGetP(dVar);
                }
...
public:
        ...
        Read(const DataManager* pDM, MBDynParser& HP) override {
                ...
                SAFENEWWITHCONSTRUCTOR(pTplDC,
                        ...
                        CompTplDriveCaller<T>(pDM->pGetDrvHdl(), std::move(dc)));
...
template <class T>
struct DrivesArrayBase {
     explicit DrivesArrayBase(DriveCaller* pDC = nullptr)
          : pDriveCaller(pDC) {
     }

     DrivesArrayBase(DrivesArrayBase&& oDR)
          : pDriveCaller(std::move(oDR.pDriveCaller)) {
     }

     bool bIsDifferentiable() const {
          return pDriveCaller->bIsDifferentiable();
     }

protected:
     std::unique_ptr<DriveCaller> pDriveCaller;
};
...
struct DrivesArray: public DrivesArrayBase<T> {
     ...
     DrivesArray(DrivesArray&& oDR)
          :DrivesArrayBase<T>(std::move(oDR)), t(std::move(oDR.t)) {
     }

     T Get(const doublereal& dVar) const {
          return t * this->pDriveCaller->dGet(dVar);
     }

     T GetP(const doublereal& dVar) const {
          return t * this->pDriveCaller->dGetP(dVar);
     }

     DrivesArray pCopy() const {
          return DrivesArray(this->pDriveCaller->pCopy(), t);
     }

     std::ostream& Restart(std::ostream& out) const {
          Write(out, t, ", ");
          out << ", ";
          return this->pDriveCaller->Restart(out);
     }
...
public:
        explicit ArrayTplDriveCaller(const DriveHandler* pDH, std::vector<DrivesArray<T> >&& dc)
             : TplDriveCaller<T>(pDH), m_dc(std::move(dc)) {
        }
        ...
        /* copia */
        virtual TplDriveCaller<T>* pCopy(void) const override {
                std::vector<DrivesArray<T> > dc;

                dc.reserve(m_dc.size());

                for (const auto& pDC: m_dc) {
                     dc.emplace_back(pDC.pCopy());
                }

                typedef ArrayTplDriveCaller<T> dc_t;

                TplDriveCaller<T>* pDC = nullptr;

                SAFENEWWITHCONSTRUCTOR(pDC, dc_t, dc_t(this->pDrvHdl, std::move(dc)));
...
public:
        virtual TplDriveCaller<T> *
        Read(const DataManager* pDM, MBDynParser& HP) override {
                const integer iNumDr = HP.GetInt();
                if (iNumDr <= 0) {
                ...
                for (integer i = 0; i < iNumDr; i++) {
                     T t = GetT(HP, mb_zero<T>());
                     dc.emplace_back(HP.GetDriveCaller(), t);
                }

                TplDriveCaller<T>* pTplDC = nullptr;

                SAFENEWWITHCONSTRUCTOR(pTplDC,
                        ArrayTplDriveCaller<T>,
                        ArrayTplDriveCaller<T>(pDM->pGetDrvHdl(), std::move(dc)));
