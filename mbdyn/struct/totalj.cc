        ...
        SimulationEntity::Hints *ph)
{
        ...
        for (unsigned iCnt = 0; iCnt < nPosConstraints; iCnt++) {
                X(iFirstReactionIndex + 1 + iPosEqIndex[iCnt]) = F(iPosIncid[iCnt]);
        }

        for (unsigned iCnt = 0; iCnt < nRotConstraints; iCnt++) {
                X(iFirstReactionIndex + 1 + iRotEqIndex[iCnt]) = M(iRotIncid[iCnt]);
        }

        for (unsigned iCnt = 0; iCnt < nVelConstraints; iCnt++) {
                X(iFirstReactionIndex + 1 + iVelEqIndex[iCnt]) = F(iVelIncid[iCnt]);
        }

        for (unsigned iCnt = 0; iCnt < nAgvConstraints; iCnt++) {
                X(iFirstReactionIndex + 1 + iAgvEqIndex[iCnt]) = M(iAgvIncid[iCnt]);
        }
}
...
void TotalJoint::Restart(RestartData& oData, RestartData::RestartAction eAction)
{
...
void TotalPinJoint::Restart(RestartData& oData, RestartData::RestartAction eAction)
{
     oData.Sync(RestartData::ELEM_JOINTS, GetLabel(), "F", F, eAction);
     oData.Sync(RestartData::ELEM_JOINTS, GetLabel(), "M", M, eAction);
}
